 ...
 * @since 1.0
 */
class IsABehavior extends \yii\base\Behavior
{
    /**
    ...
    /**
     * Execute before model saved
     * @param \yii\base\Event $event
     */
    public function beforeSave($event){

        if(!$this->_relation->save()){
            echo '<br>Error: Could Not Save <br>';
            echo $this->_relation->className();
            echo '<br>Because Save <br>';
            var_dump($this->_relation->getErrors());
            Yii::$app->end();
        }
        foreach ($this->relationKey as $from => $to) {
